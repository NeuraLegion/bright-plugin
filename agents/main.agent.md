...
5. Use the Bright MCP-supported `addAuth` tool for standard session, JWT, API-key, HTML-form CSRF, OAuth2 or PKCE, and multistep flows. Use the `addAuth` schema, request structure, and expected parameters from `https://github.com/NeuraLegion/services/tree/main/apps/mcp`. `testAuth` is the source of truth; recreate broken auth objects instead of layering guesses.
