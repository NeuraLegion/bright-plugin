...
argument-hint: "Mode (full or function), scope (whole app or PR scope of specific service), and goal (scan-only or remediate). Add auth/setup hints, areas to focus or exclude, and any hard constraints."
mcp-servers:
  bright:
    type: http
    ...
    oidc:
      ...
      endpoints:
        exchange: 'https://app.brightsec.com/api/v1/token'
        revoke: 'https://app.brightsec.com/api/v1/revoke'
  cli:
    type: stdio
    command: npx
...
3. Create or connect a Repeater.
    - For local targets, ensure a Repeater is available. If you do not already know a suitable existing Repeater ID, call `createRepeater` (named with the `bright-agent-` prefix). Then call `runRepeater` with that ID and `app.brightsec.com` hostname to start it.
...
`https://app.brightsec.com/projects/{projectId}/scans/{scanId}/issues/{vulnerabilityId}`
